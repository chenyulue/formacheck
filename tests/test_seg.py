import pytest
from formacheck.seg import extract_technical_feature
...
@pytest.mark.parametrize("claim, expected", [
    ...
    ("4.如权利要求3所述的套刻精度误差检测方法，其特征在于，对初始量测版图进行形态学开运算处理的方法包括：对上述的实际图形先进行腐蚀处理，并在腐蚀处理之后进行膨胀处理，获取第一初始量测版图；对第一初始量测版图进行提取轮廓处理，前述提取轮廓处理用于获取实际图形的轮廓；在提取轮廓处理后，对前述的实际图形的轮廓进行最小面积筛选处理，该最小面积筛选处理用于去除实际图形的轮廓中的无效区域，获取第二过渡版图。",
     {'套刻精度误差检测方法': [11], '实际图形': [55, 144], '提取轮廓处理': [113], '最小面积': [163]}),
    ("4.如权利要求3所述的制备方法，其中上述GaN层由CVD工艺淀积形成。",
     {'制备方法': [11], 'GaN层': [20]}),
    ("2.根据权利要求1所述的常关型栅控结型场效应晶体管，其特征在于，所述沟道区（1）的竖向外边缘和所述栅区（5）的竖向外边缘在竖向方向对齐，所述沟道区（1）掺杂有杂质。",
     {'常关型栅控结型场效应晶体管': [12], '沟道区': [34, 70], '栅区': [49]})

])
def test_extract_technical_feature(claim, expected):
    assert extract_technical_feature(claim) == expected